...
export default function Permissions() {
  ...
  const handleSubmit = async () => {
    const selectedPermissions = Object.keys(toggles).filter((key) => toggles[key]);
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white shadow-md p-6 rounded-lg">
        ...
        {permissions.map((perm, idx) => (
          <div key={idx} className="mb-6">
            <h3 className="font-semibold text-md mb-3 bg-gray-200 px-4 py-2 rounded-lg">{perm.category}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {perm.options.map((option, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between bg-gray-50 px-4 py-3 rounded-lg border shadow-sm"
                >
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={() => handleToggle(option)}
                    />
                    <div className="w-11 h-6 bg-gray-300 peer-focus:outline-none rounded-full peer peer-checked:bg-green-500 relative transition-colors">
