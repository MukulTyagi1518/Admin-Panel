...
const TaxTable = () => {
    ...
    const [newTaxName, setNewTaxName] = useState("");
    const [taxData, setTaxData] = useState([
        { id: 1, name: "VAT", status: true },
        { id: 2, name: "GST", status: false },
        { id: 3, name: "TAX", status: false },
    ]);
    ...
    const handleSave = async () => {
        if (newTaxName.trim() !== "") {
            try {
                // Sending the POST request to your backend API
                const response = await axios.post("http://localhost:5000/api/vatTax/create", {
                    name: newTaxName,
                    status: true, // Adding status field, as expected by backend
                });

                // Log the response to verify what is returned from the backend
                console.log('Tax Saved:', response.data);

                // Assuming the response returns the newly created tax
                const newTax = response.data.data;

                // Check if response data has id and name
                if (newTax && newTax._id && newTax.name) {
                    setTaxData((prev) => [
                        ...prev,
                        { id: newTax._id, name: newTax.name, status: newTax.status },
                    ]);
                    setNewTaxName(""); // Reset the input field
