...
const RejectedRequests = () => {
  ...
  // Fetch rejected refund requests
  useEffect(() => {
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="bg-white rounded-lg overflow-hidden">
        <h2 className="text-xl font-semibold px-4 py-3 border-b">Rejected Requests</h2>
        ...
        {loading ? (
          ...
          <>
              ...
              <table className="min-w-full bg-white border border-gray-200">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="px-4 py-2 border">Order Code</th>
                    <th className="px-4 py-2 border">Seller Name</th>
                    <th className="px-4 py-2 border">Product</th>
                    <th className="px-4 py-2 border">Price</th>
                    <th className="px-4 py-2 border">Seller Approval</th>
                    <th className="px-4 py-2 border">Admin Approval</th>
                    <th className="px-4 py-2 border">Reject Reason</th>
                  </tr>
                </thead>
                <tbody>
                  {requests.map((request, index) => (
                    <tr key={request._id || request.id} className="border-t">
                      <td className="px-4 py-2 border">{index + 1}</td>
                      <td className="px-4 py-2 border">{request.orderCode}</td>
                      <td className="px-4 py-2 border">{request.sellerName}</td>
                      <td className="px-4 py-2 flex items-center space-x-3">
                        ...
                        />
                        <span className="text-blue-500">{request.productName}</span>
                      </td>
                      <td className="px-4 py-2 border">{request.price}</td>
                      <td className="px-4 py-2 border text-center">
                        <span className="px-2 py-1 text-white bg-blue-500 rounded text-xs">
                          {request.sellerApproval}
                        </span>
            ...
            <div className="block md:hidden">
              {requests.map((request) => (
                <div key={request._id || request.id} className="border-b py-3">
                  <div className="flex items-center px-4 py-2 space-x-4">
                    <button
                      onClick={() => toggleRow(request._id || request.id)}
                      className="bg-gray-200 p-2 rounded-full"
                    >
                      {expandedRows[request._id || request.id] ? <FaMinus /> : <FaPlus />}
                    </button>
                    <div className="flex items-center space-x-4">
                      <img
                        src={request.productImage}
                  ...
                  {expandedRows[request._id || request.id] && (
                    <div className="bg-gray-100 p-4 rounded-md mt-3">
                      <div className="grid grid-cols-2 gap-y-3">
                        <span className="font-semibold">Order Code:</span>
                        <span>{request.orderCode}</span>
