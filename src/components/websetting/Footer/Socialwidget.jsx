...
export default function Socialwidget({
  FaBold, FaItalic, FaUnderline, FaListUl, FaListOl, MdOutlineFormatClear, MdFormatColorText,
  FaTable, FaLink, FaImage, FaVideo, FaCode, FaUndo, FaRedo,
}) {
  const [copyrightText, setCopyrightText] = useState("");
  const [showLinks, setShowLinks] = useState(true);
  const [sellerAppLink, setSellerAppLink] = useState("");
  ...
  const handleUpdate = async () => {
    try {
      ...
      formData.append("showSocialLinks", showLinks);
      formData.append("sellerAppLink", sellerAppLink);
      formData.append("socialLinks", JSON.stringify(
        Object.fromEntries(
          socialLinks.map((link, index) => [`link${index + 1}`, link.url])
        )
      ));
      ...
      const response = await axios.post("https://e-commerce-backend-1-0.onrender.com/api/footerbottom", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      alert("Footer Bottom Updated!");
  ...
  return (
    <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 ml-4 mr-4 mb-4">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 mb-10 ml-4 mr-4">
        ...
        <label className="md:w-1/4 font-medium text-md text-gray-700">Copyright Text</label>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 mb-10 ml-4 mr-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          >
            <div className={`bg-white w-4 h-4 rounded-full shadow-md transform duration-300 ${showLinks ? "translate-x-5" : "translate-x-0"}`} />
          </div>
        </div>

        <div className="space-y-3">
          {socialLinks.map((link, index) => (
            <div key={index} className="flex items-center border border-gray-300 rounded-md overflow-hidden bg-white">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl mt-6 p-6 mb-10 ml-4 mr-4">
        ...
        <div className="mb-4">
          ...
          <input
            type="text"
            value={sellerAppLink}
            onChange={(e) => setSellerAppLink(e.target.value)}
