...
export default function Custom({ handleUpdate }) {
  ...
  const handlePostScript = async () => {
    try {
      await axios.post("https://e-commerce-backend-1-0.onrender.com/api/custom-script", {
        headerScript,
        footerScript,
      });
      handleUpdate(); // Optional: notify success externally
