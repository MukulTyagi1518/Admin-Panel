...
const NotificationTypes = () => {
  ...
  const toggleStatus = (id) => {
    setNotificationTypes(
      notificationTypes.map((item) => (item.id === id ? { ...item, status: !item.status } : item))
    );
  };

  const deleteNotification = (id) => {
    setNotificationTypes(notificationTypes.filter((item) => !item.isDefault && item.id !== id));
  };

  const filteredNotifications = notificationTypes.filter(
    (item) =>
      item.userType === activeTab && item.type.toLowerCase().includes(searchTerm.toLowerCase())
  );
